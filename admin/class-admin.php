...
class Admin {
	...
	private function dependencies() {
		...
		require_once YATES_PATH . 'admin/class-settings.php';

		// Add icons to the titles of ACF tab and accordion fields, if active.
		if ( class_exists( 'acf_pro' ) && ! get_option( 'yates_acf_activate_settings_page' ) ) {
			include_once YATES_PATH . 'admin/class-acf-tab-icons.php';
		}

		// Include custom fields for Advanced Custom Fields Pro, if active.
		if ( class_exists( 'acf_pro' ) && ! get_option( 'yates_acf_activate_settings_page' ) ) {
			include_once YATES_PATH . 'admin/class-settings-fields-site-acf.php';
		}

		// Functions for dasboard widgets, excluding the welcome panel.
		require_once YATES_PATH . 'admin/dashboard/class-dashboard.php';

		// Functions for admin menu item positions and visibility.
		require_once YATES_PATH . 'admin/class-admin-menu.php';

		// Add menus to the admin toolbar.
		require_once YATES_PATH . 'admin/class-admin-toolbar-menus.php';

		// Functions for various admin pages and edit screens.
		require_once YATES_PATH . 'admin/class-admin-pages.php';

		// Import custom fields for editing, if ACF Pro is active.
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			include_once YATES_PATH . 'admin/class-fields-import.php';
		}

		// Filter by page template.
		require_once YATES_PATH . 'admin/class-admin-template-filter.php';
	...
	public function admin_footer() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			 ...
			 */
			$credit = get_field( 'yates_admin_footer_credit', 'option' );
			$link   = get_field( 'yates_admin_footer_link', 'option' );
			...
			if ( $credit && $link ) {
				$footer = sprintf(
					...
					$site,
					esc_html__( 'website designed & developed by', 'yates' ),
					esc_url( $link ),
					$credit
				);
			...
			// If no input we use the name of the site.
				...
				$footer = sprintf(
					'%1s %2s. ',
					$site,
					esc_html__( 'website powered by WordPress', 'yates' )
				);
			}
		 ...
		 */
			...
			$credit = sanitize_text_field( get_option( 'yates_footer_credit' ) );
			$link   = esc_url_raw( get_option( 'yates_footer_link' ) );
		...
		$admin_footer = apply_filters( 'yates_admin_footer', $footer );
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( YATES_ADMIN_SLUG . '-admin', YATES_URL . 'admin/assets/css/admin.min.css', [], YATES_VERSION, 'all' );
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( YATES_ADMIN_SLUG . '-tooltips', YATES_URL . 'admin/assets/css/tooltips.min.css', [], YATES_VERSION, 'all' );
		 ...
		 */
		if ( class_exists( 'acf' ) ) {
			wp_enqueue_style( YATES_ADMIN_SLUG . '-acf', YATES_URL . 'admin/assets/css/acf.css', [], YATES_VERSION, 'all' );
		}
		...
		if ( $welcome ) {
			wp_enqueue_style( YATES_ADMIN_SLUG . '-welcome', YATES_URL . 'admin/assets/css/welcome.css', [], YATES_VERSION, 'all' );
		}
	...
	public function get_search_form() {
		...
		require YATES_PATH . 'partials/searchform.php';
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( YATES_ADMIN_SLUG . '-conditionalize', YATES_URL . 'admin/assets/js/admin.js', [ 'jquery' ], YATES_VERSION, true );

		// Enqueue scripts for backend functionality of this plugin.
		wp_enqueue_script( YATES_ADMIN_SLUG . '-admin', YATES_URL . 'admin/assets/js/conditionize.flexible.jquery.min.js', [ 'jquery' ], YATES_VERSION, true );
